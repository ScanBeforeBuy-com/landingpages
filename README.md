...
Redirect the old URL to the closest relevant new page.
Update all internal links.
Update the canonical.
Update structured-data URLs.
Add the final destination URL to the sitemap.
Robots.txt
...
Open every new page manually.
Confirm the intended clean URL loads.
Confirm the canonical matches the browser URL.
Test the trailing-slash version.
Test /index.html.
Test any possible .html version.
Confirm duplicate variants redirect.
Confirm the final page returns HTTP 200.
Confirm internal links work.
Confirm CTA links work.
Inspect the HTML source.
Confirm the title and meta description.
Confirm schema URLs are correct.
Confirm visible FAQs match FAQ schema.
Open the sitemap.
Confirm the new URLs are listed.
Open robots.txt.
...
Submit or resubmit:
https://www.scanbeforebuy.com/sitemap.xml
Inspect the main hub pages.
Inspect the strongest buyer guides.
Inspect the most commercially relevant pages.
Run live URL tests.
Request indexing for priority pages.
Confirm Google can access each page.
Confirm Google detects the intended canonical.
Monitor indexing, crawling, and canonical reports.
Review impressions, clicks, CTR, and average position.
Improve pages that receive impressions but few clicks.
Strengthen pages ranking between positions 8 and 30.
Consolidate pages competing for the same search intent.
Rewrite or remove weak pages that remain thin or duplicative.
Priority Indexing
